...
from deputydev_core.services.mcp.dataclass.main import (
    ...
    ConnectionStatus,
    Tools,
    ToolInvokeRequest, McpDefaultSettings,
)
import mcp
...
class McpService:
    ...
    @classmethod
    async def init(cls, mcp_config_path):
        if not cls.mcp_client:
            settings = cls.get_default_settings()
            cls.mcp_client = await MCPClient.get_instance(default_settings=settings,
                                                          mcp_config_path=mcp_config_path)

    @classmethod
    @handle_mcp_exceptions
    async def sync_mcp_servers(cls, config_path):
    ...
    async def invoke_tool(
            cls, tool_invoke_request: ToolInvokeRequest
    ...
    @classmethod
    async def create_server_list(cls, limit, offset):
        servers = []
        ...
        eligible_servers = cls.mcp_client.get_servers(
    ...
    @classmethod
    @handle_mcp_exceptions
    async def get_servers(cls, limit=-1, offset=0):
        if not cls.mcp_client:
            return []
