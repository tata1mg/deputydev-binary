...
class RelevantChunksService:
    ...
    async def get_relevant_chunks(cls, payload):
        ...
        with ProcessPoolExecutor(max_workers=NUMBER_OF_WORKERS) as executor:
            weaviate_client = await InitializationManager(repo_path=repo_path, auth_token=auth_token,
                                                          process_executor=executor,
                                                          one_dev_client=one_dev_client).initialize_vector_db()
            max_relevant_chunks = ConfigManager.configs["CHUNKING"]["NUMBER_OF_CHUNKS"]
            reranked_chunks, input_tokens, focus_chunks_details = await ChunkingManger.get_relevant_chunks(
                query=query,
