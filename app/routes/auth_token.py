...
async def store_token(_request: Request, **kwargs):
    response = await AuthTokenService.store_token(get_context_value("headers").get("X-Client"))
...
async def load_token(_request: Request, **kwargs):
    response = await AuthTokenService.load_token(get_context_value("headers").get("X-Client"))
    return json(response)
